...
<article class="max-w-screen-md mt-5 py-6 px-6 border-t-gray-600 border-b-4 border-l-4 border-purple-200 rounded-md">
  ...
  <div class="font-extralight text-sm">
    By. <a href="/author/{{ $post->author->username }}" class="hover:underline text-blue-600">{{ $post->author->name }}</a> in <a class="text-blue-600 hover:underline" href="/category/{{ $post->category->slug }}">{{ $post->category->name }}</a> {{ $post->created_at->diffForHumans() }}
  </div>
